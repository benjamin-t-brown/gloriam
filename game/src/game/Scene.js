...
import { normalizeClamp, pt, HEADING } from 'utils';
...
class Scene {
  ...
  getInventory(actorName) {
    return (
      (
        this.storage[actorName] && Object.keys(this.storage[actorName].items)
      ).filter(itemName => this.storage[actorName].items[itemName]) || []
    );
  }
  ...
  getActiveItem() {
    return this.storage.activeItem;
  }

  setActiveItem(itemName) {
    this.storage.activeItem = itemName;
  }

  unsetActiveItem(itemName) {
    this.storage.activeItem = '';
  }
  ...
  evalCondition(conditional) {
    if (conditional === true) {
      ...
      if (type === 'is') {
        ...
        const arg =
          args[0] || (this.currentScript || this.currentTrigger).name + '-once';
